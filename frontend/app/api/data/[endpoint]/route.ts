...
function fallbackData(endpoint: string) {
  ...
  if (endpoint === 'predict') {
    return [
      { scenario: 'Baseline', value: 2.9 },
      { scenario: 'Dry season stress', value: 2.4 },
      { scenario: 'Irrigation support', value: 3.5 }
    ];
  }
...
export async function GET(_: NextRequest, { params }: { params: { endpoint: string } }) {
  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json({ data: fallbackData(endpoint), warning: `Backend request failed (${response.status}). Showing fallback data.` });
    }

    return NextResponse.json(body);
  } catch {
    return NextResponse.json({ data: fallbackData(endpoint), warning: 'Backend unreachable. Showing fallback data.' });
  }
}
